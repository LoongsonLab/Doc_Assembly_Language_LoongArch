...
【例3】 la 指令示例：PC 相对寻址场景

当目标符号在 PC 相对寻址范围内（±1GB）时，`la` 通常转换为：
...
【例6】 la 指令示例：GOT（Global Offset Table）加载

在位置无关代码（PIC）中，需要通过 GOT 访问全局符号：
...
【例7】 la 指令示例：TLS（Thread-Local Storage）加载
...
LoongArch64 支持动态大小端切换，符号加载需要考虑字节序：
